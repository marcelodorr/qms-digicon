...
namespace backend.Controllers
{
    ...
    public class NormaController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update([FromBody] NormaModel payload)
        {
            ...
            {
                entity.Norma = Truncate(payload.Norma, ResolveLength(lengths, "Norma"));
            }

            if (!string.IsNullOrWhiteSpace(payload.Revision))
            {
                entity.Revision = Truncate(payload.Revision, ResolveLength(lengths, "Revision"));
            }
        ...
        public async Task<IActionResult> ImportFromExcel([FromForm] IFormFile? file, [FromForm] string? createdBy)
        {
            ...
            var items = rows
                .Select(r =>
                {
                    ...
                    {
                        Cliente = Truncate(client, ResolveLength(lengths, "Cliente")),
                        Processo = Truncate(process, ResolveLength(lengths, "Processo")),
                        Norma = Truncate(standard, ResolveLength(lengths, "Norma")),
                        Revision = Truncate(revision, ResolveLength(lengths, "Revision")),
