...
namespace backend.Controllers
{
    ...
    public class PartNumberController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] PartNumberModel payload)
        {
            ...
            var descricao = payload?.Descricao?.Trim();
            ...
            var entity = new PartNumberModel
            {
                ...
                ClienteNome = string.IsNullOrWhiteSpace(client?.Cliente)
                    ? null
                    : client!.Cliente.Trim().ToUpperInvariant(),
        ...
        public async Task<IActionResult> Update([FromBody] PartNumberModel payload)
        {
            ...
            // Atualiza descrição: se vazio, coloca "-", senão usa o valor trimado
            var novaDescricao = payload.Descricao?.Trim();
            ...
            {
                entity.Descricao = novaDescricao;
            }
            ...
            if (payload.ClienteId.HasValue)
            {
                ...
                entity.ClienteNome = string.IsNullOrWhiteSpace(client.Cliente)
                    ? null
                    : client.Cliente.Trim().ToUpperInvariant();
            }
        ...
        public async Task<IActionResult> ImportFromExcel([FromForm] IFormFile? file, [FromForm] string? createdBy)
        {
            ...
            var items = rows
                .Select(r =>
                {
                    var clientName = r.Get("Cliente") ?? r.Get("Client");
                    return new PartNumberModel
                    {
                        ...
                        ClienteNome = string.IsNullOrWhiteSpace(clientName)
                            ? null
                            : clientName.Trim().ToUpperInvariant(),
