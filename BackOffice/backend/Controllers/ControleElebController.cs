...
namespace backend.Controllers
{
    ...
    public class ControleElebController : ControllerBase
    {
        ...
        [HttpGet("por-po")]
        public async Task<IActionResult> GetByPoQuery([FromQuery] string poEleb)
        {
            if (string.IsNullOrWhiteSpace(poEleb))
            {
                return BadRequest(new { message = "PO inválida." });
            }
            ...
            {
                _logger.LogError(ex, "Falha em GET /api/ControleEleb/por-po?poEleb={poEleb}", poEleb);
                return Problem(title: "Erro ao buscar a PO", statusCode: 500);
            }
        }

        [HttpGet("detalhes/{orderNumber}")]
        public async Task<IActionResult> GetDetailsByOrder(string orderNumber)
        {
            if (string.IsNullOrWhiteSpace(orderNumber))
            {
                return BadRequest(new { message = "Ordem inválida." });
            }

            try
            {
                var details = await _service.GetDetailsByOrderAsync(orderNumber);
                return Ok(details);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Falha em GET /api/ControleEleb/detalhes/{orderNumber}", orderNumber);
                return Problem(title: "Erro ao buscar detalhes da ordem", statusCode: 500);
            }
        }
