...
namespace backend.Controllers
{
    ...
    public class SpecialProcessCertificateController : ControllerBase
    {
        ...
        public record SpecialProcessPdfBatchRequest(List<int>? Ids, string? Disposition = null, bool? SaveOnServer = null);
        ...
        [HttpPost("gerar-pdf-combinado")]
        public async Task<IActionResult> GenerateCombinedPdf([FromBody] SpecialProcessPdfBatchRequest req)
        {
            if (req?.Ids == null || req.Ids.Count == 0)
                return BadRequest(new { message = "Ids dos certificados são obrigatórios." });
            ...
            try
            {
                var result = await _service.GenerateCombinedPdfAsync(req.Ids, saveOnServer);
