...
namespace backend.Services
{
    ...
    public class ProductConformityCertificateService
    {
        ...
        public async Task<ProductConformityPdfResult> GeneratePdfAsync(int certificateId, bool saveOnServer = false)
        {
            ...
            var parameterRevision = await ResolveParameterRevisionByPartNumberAsync(cert.PartNumber);
            var partRev = string.IsNullOrWhiteSpace(parameterRevision)
                ? (string.IsNullOrWhiteSpace(cert.PartNumberRevision)
            ...
            var fileName = $"Product_CofC_{SanitizeFileSegment(cert.CertificateNumber)}_{cert.EmissionDate:yyyyMMdd}.pdf";
            byte[] fileBytes;
            ...
            docInfoParagraph.Add(new Chunk("\n"));
            docInfoParagraph.Add(new Chunk(docControl.DocumentDate.ToString("dd/MM/yyyy"), valueFont));
            ...
            var dateCell = new PdfPCell(new Paragraph($"Date: {cert.EmissionDate.ToString("dd/MM/yyyy", culture)}", valueFont))
            {
                Border = Rectangle.NO_BORDER,
                HorizontalAlignment = Element.ALIGN_CENTER,
                PaddingBottom = 2f
