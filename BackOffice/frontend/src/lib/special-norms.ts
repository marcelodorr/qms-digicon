...
export type SpecialNormHistoryItem = {
  id: string;
...
export async function fetchSpecialNormHistory(specialProcessId: string): Promise<SpecialNormHistoryItem[]> {
  const list = await requestJson<Record<string, unknown>[]>(`/api/SpecialProcess/${specialProcessId}/history`);
  if (!Array.isArray(list)) return [];
  return list.map(item => {
    const raw = item as Record<string, unknown>;
    return {
      id: String(raw.Id ?? raw.id ?? ""),
      specialProcessId: String(raw.SpecialProcessId ?? raw.specialProcessId ?? specialProcessId),
...
export async function updateSpecialNorm(
  ...
  const payload = buildPayload(specialProcess, specification, revision, comment, createdBy, id, observation);
  const response = await requestJson<SpecialNormApiResponse>("/api/SpecialProcess", {
    method: "PUT",
    body: JSON.stringify(payload),
  });
