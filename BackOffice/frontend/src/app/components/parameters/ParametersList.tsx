...
export function ParametersList({ 
  ...
  const filteredData = data.filter(p => {
    ...
    return (
      ...
      p.norm.toLowerCase().includes(term) ||
      (p.normaRevision ?? '').toLowerCase().includes(term)
    );
  });

  const columns = React.useMemo<GridColDef<Parameter>[]>(() => {
    const baseColumns: GridColDef<Parameter>[] = [
      {
        field: 'partNumber',
      ...
      },
      {
        ...
        renderCell: (params) => (
          <div className="flex flex-col text-xs text-slate-500">
            ...
            <span>{params.row.updatedBy}</span>
          </div>
        ),
      },
    ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">Cadastro de Parâmetros</h2>
          <p className="text-slate-500">Defina parâmetros de controle vinculados a processos e normas.</p>
        </div>
        {canManage && (
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              >
                ...
                <div className="flex gap-2 mt-2">
                  <label htmlFor="param-file-upload" className="cursor-pointer">
      ...
      <div className="flex items-center gap-2 bg-white p-2 rounded-md border max-w-sm">
        ...
        <Input 
          placeholder="Buscar por parâmetro, PN, processo ou norma..." 
          value={searchTerm}
