...
import type { SpecialNorm } from '../../../lib/special-norms';

interface SpecialNormsListProps {
  data: SpecialNorm[];
  onEdit: (norm: SpecialNorm) => void;
...
export function SpecialNormsList({ 
  data, 
  onEdit, 
  onDelete, 
  onDeleteSelected,
  ...
  const filteredData = data.filter(norm => 
    norm.specialProcess.toLowerCase().includes(searchTerm.toLowerCase()) ||
    norm.specification.toLowerCase().includes(searchTerm.toLowerCase()) ||
    norm.revision.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const columns = React.useMemo<GridColDef<SpecialNorm>[]>(() => {
    const baseColumns: GridColDef<SpecialNorm>[] = [
      {
        field: 'specialProcess',
        headerName: 'Processo Especial',
        flex: 1,
        minWidth: 200,
        renderCell: (params) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">Normas Processo Especial</h2>
          <p className="text-slate-500">Gerencie normas específicas para processos especiais.</p>
        </div>
        {canManage && (
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              >
                ...
                <div className="flex gap-2 mt-2">
                  <label htmlFor="sn-file-upload" className="cursor-pointer">
      ...
      <div className="flex items-center gap-2 bg-white p-2 rounded-md border max-w-sm">
        ...
        <Input 
          placeholder="Buscar processo, especificação ou revisão..." 
          value={searchTerm}
      ...
      <div className="rounded-md border bg-white">
        <QmsDataGrid
          ...
          columns={columns}
          loading={isLoading}
          emptyMessage="Nenhuma norma encontrada."
          loadingMessage="Carregando normas especiais..."
