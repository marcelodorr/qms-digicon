...
const normalizeFileName = (name: string) => {
  ...
  const replaced = trimmed.replace(/\.xlsx$/i, ".csv");
  return replaced.toLowerCase().endsWith(".csv") ? replaced : `${trimmed}.csv`;
};
...
function ExportToolbar({
  ...
  const handleExport = () => {
    ...
    const csvContent = buildCsvContent([headerRow, ...dataRows]);
    downloadFile(csvContent, normalizeFileName(fileName), "text/csv;charset=utf-8");
  };
...
export function QmsDataGrid({
  ...
  exportFileName = "dados_exportados.csv",
