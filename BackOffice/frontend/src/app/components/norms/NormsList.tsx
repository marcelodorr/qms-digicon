...
import type { Norm } from '../../../lib/norms';
...
export function NormsList({ 
  ...
  const filteredData = data.filter(norm => 
    norm.client.toLowerCase().includes(searchTerm.toLowerCase()) ||
    norm.standard.toLowerCase().includes(searchTerm.toLowerCase()) ||
    norm.process.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const columns = React.useMemo<GridColDef<Norm>[]>(() => {
    const baseColumns: GridColDef<Norm>[] = [
      {
        field: 'client',
        headerName: 'Cliente',
        flex: 1,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">Cadastro de Normas</h2>
          <p className="text-slate-500">Gerencie normas, revisões e vínculos com clientes e processos.</p>
        </div>
        {canManage && (
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              >
                ...
                <div className="flex gap-2 mt-2">
                  <label htmlFor="norm-file-upload" className="cursor-pointer">
      ...
      <div className="flex items-center gap-2 bg-white p-2 rounded-md border max-w-sm">
        ...
        <Input 
          placeholder="Buscar por cliente, norma ou processo..." 
          value={searchTerm}
      ...
      <div className="rounded-md border bg-white">
        <QmsDataGrid
          ...
          loadingMessage="Carregando normas..."
