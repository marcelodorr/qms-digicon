...
import { useQualityCatalog } from '@/features/quality-catalog/hooks';
...
function NcDialog({
  ...
  const { defects, causes } = useQualityCatalog();
  const [defect, setDefect] = useState<Defect | null>(null);
  useEffect(() => { if (!defect && defects.length) setDefect(defects.find((d) => d.code === 'D06') ?? defects[0]); }, [defects, defect]);
